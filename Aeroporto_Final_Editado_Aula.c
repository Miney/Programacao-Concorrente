...
struct aeroporto{
    ...
    int n_equipes;
    int n_aeroporto;
    int vetor_portoes[portoes];
    Equipe *vetor_equipes[equipes];
...
int main(){
    ...
    for(i=0;i<n_aeroportos;i++){
        ...
        int vetor_parametros[2];
        vetor_parametros[0]=i;
        for(j=0;j<equipes;j++){
            vetor_parametros[1]=j;
            Aeroportos[i]->vetor_equipes[j]=(Equipe*)malloc(sizeof(Equipe));
            Aeroportos[i]->vetor_equipes[j]->estado=aberto;
            sem_init(&Aeroportos[i]->vetor_equipes[j]->semaforo_equipe,0,0);

            if(pthread_create(&Aeroportos[i]->vetor_equipes[j], NULL, &thread_equipe, (void*)vetor_parametros))
            {
...
void* thread_aeronave (void *arg){
    ...
    while(1){
        ...
        sem_post(&Aeroportos[n_aeroporto]->vetor_equipes[equipe]->semaforo_equipe);
        ...
        sem_wait(&aeronaves[localizador]->semaforo_aeronave);
...
void* thread_equipe (void *arg){
    ...
    int parametros_equipe[2]=(int)*arg;
    int equipe, aeroporto;
    while(1){
        ///checar se o tempo de preparacao e random ou se ele e pre definido(qualquer coisa so usar funcao pra gerar random)
        sem_wait(&Aeroporto[aeroporto]->vetor_equipes[equipe]->semaforo_equipe);
        printf("Esquipe %d esta preparando a aeronave %d no aeroporto %d\n", localizador_equipe);
...
Aeronave* inicializaAeronave(int localizador){
    ...
    sem_init(&Aeronave_inicializada->semaforo_aeronave,0,0);
...
int pede_equipe(int n_aeroporto,int n_aeronave){
    ...
    while(1){
        sem_wait(&Aeroportos[n_aeroporto]->sem_contador_equipes);
        int equipes_ocupadas=Aeroportos[n_aeroporto]->cont_equipes_ocupadas;
        sem_post(&Aeroportos[n_aeroporto]->sem_contador_equipes);
        if(equipes_ocupadas<equipes){

            sem_wait(&Aeroportos[n_aeroporto]->sem_contador_equipes);
            for(i=0;i<equipes;i++){
                if(Aeroportos[n_aeroporto]->vetor_equipes[i]->estado==livre){


                    Aeroportos[n_aeroporto]->cont_equipes_ocupadas++;

                    Aeroportos[n_aeroporto]->vetor_equipes[i]->estado=fechado;
